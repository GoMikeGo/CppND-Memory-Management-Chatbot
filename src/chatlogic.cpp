...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
    ...
    ChatBot init_chatBot("../images/chatbot.png");
  
    // add pointer to chatlogic so that chatbot answers can be passed on to the GUI
    init_chatBot.SetChatLogicHandle(this);
  
    // add chatbot to graph root node
    init_chatBot.SetRootNode(rootNode);

    // set pointer to chatbot to pass user answer to chatbot
 //   SetChatbotHandle(&init_chatBot); 
    
    //move ChatBot to rootNode 
    rootNode->MoveChatbotHere(std::move(init_chatBot));
